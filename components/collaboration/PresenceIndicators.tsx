...
export function PresenceIndicators({ presenceList, currentUserId }: PresenceIndicatorsProps) {
  ...
  return (
    <div className="absolute top-4 right-4 flex items-center gap-2 bg-secondary/80 backdrop-blur-sm rounded-md shadow-elevation-2 border border-border p-2">
      <span className="text-sm text-foreground-secondary mr-2">Active:</span>
      {otherUsers.map((user) => (
        <div
          key={user.userId}
        ...
        >
          ...
          <span className="text-small font-medium" style={{ color: user.color }}>
            {user.userName}
          </span>
        </div>
      ))}
